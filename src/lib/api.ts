  ...
  async getCompanyFlights(airlineId: string): Promise<ApiResponse<Flight[]>> {
    return handleApiResponse(
      apiClient.get(`/flights/company/${airlineId}`).then((response) => ({
        ...response,
        data: response.data.map((flight: any) => ({
          id: flight.id,
      ...
      apiClient
        ...
        .then((response) => ({
          ...
          data: {
            ...
            flight: {
              id: response.data.flight.id,
              flightNumber: response.data.flight.flight_number,
              airlineId: response.data.flight.airline_id,
              airline: response.data.flight.airline,
              origin: response.data.flight.origin,
              destination: response.data.flight.destination,
              departureTime: response.data.flight.departure_time,
              arrivalTime: response.data.flight.arrival_time,
              duration: response.data.flight.duration,
              price: response.data.flight.price,
              availableSeats: response.data.flight.available_seats,
              totalSeats: response.data.flight.total_seats,
              status: response.data.flight.status,
              createdAt: response.data.flight.created_at,
              updatedAt: response.data.flight.updated_at,
            },
            passengers: response.data.passengers.map((passenger: any) => ({
              id: passenger.id,
  ...
  async getUserBookings(): Promise<ApiResponse<Booking[]>> {
    return handleApiResponse(
      apiClient.get(`/bookings/my-bookings`).then((response) => ({
        ...response,
        data: response.data.map((booking: any) => ({
          id: booking.id,
...
export const airlineApi = {
  ...
  mapAirlineData: (airlineData: any): Airline => ({
    id: airlineData.id,
    name: airlineData.name,
    code: airlineData.code,
    logo: airlineData.logo,
    description: airlineData.description,
    isActive: airlineData.is_active !== undefined ? airlineData.is_active : airlineData.isActive,
    managerId: airlineData.manager_id || airlineData.managerId,
    createdAt: airlineData.created_at || airlineData.createdAt,
    updatedAt: airlineData.updated_at || airlineData.updatedAt,
  }),
  ...
  async getAllAirlines(): Promise<ApiResponse<Airline[]>> {
    return handleApiResponse(
      apiClient.get("/airlines").then((response) => ({
        ...response,
        data: response.data.map((airline: any) => airlineApi.mapAirlineData(airline)),
      }))
    );
  },
...
export const contentApi = {
  ...
  mapBannerData: (bannerData: any): Banner => ({
    id: bannerData.id,
    title: bannerData.title,
    description: bannerData.description,
    imageUrl: bannerData.image_url || bannerData.imageUrl,
    link: bannerData.link,
    isActive: bannerData.is_active !== undefined ? bannerData.is_active : bannerData.isActive,
    order: bannerData.order,
    createdAt: bannerData.created_at || bannerData.createdAt,
    updatedAt: bannerData.updated_at || bannerData.updatedAt,
  }),
  ...
  async getBanners(): Promise<ApiResponse<Banner[]>> {
    return handleApiResponse(
      apiClient.get("/content/banners").then((response) => ({
        ...response,
        data: response.data.map((banner: any) => contentApi.mapBannerData(banner)),
      }))
    );
  },
  ...
  async getOffers(): Promise<ApiResponse<Offer[]>> {
    return handleApiResponse(
      apiClient.get("/content/offers").then((response) => ({
        ...response,
        data: response.data.map((offer: any) => contentApi.mapOfferData(offer)),
      }))
    );
  },

  // Helper function to map offer data from backend format to frontend format
  mapOfferData: (offerData: any): Offer => ({
    id: offerData.id,
    title: offerData.title,
    description: offerData.description,
    discount: offerData.discount,
    validFrom: offerData.valid_from || offerData.validFrom,
    validTo: offerData.valid_to || offerData.validTo,
    minPrice: offerData.min_price || offerData.minPrice,
    maxDiscount: offerData.max_discount || offerData.maxDiscount,
    isActive: offerData.is_active !== undefined ? offerData.is_active : offerData.isActive,
    createdAt: offerData.created_at || offerData.createdAt,
    updatedAt: offerData.updated_at || offerData.updatedAt,
    // Optional fields that might not exist in backend
    applicableFlights: offerData.applicable_flights || offerData.applicableFlights,
    applicableAirlines: offerData.applicable_airlines || offerData.applicableAirlines,
    applicableRoutes: offerData.applicable_routes || offerData.applicableRoutes,
  }),
  ...
  async getAllUsers(page = 1, limit = 10): Promise<ApiResponse<PaginatedResponse<User>>> {
    return handleApiResponse(
      apiClient.get(`/users?skip=${(page - 1) * limit}&limit=${limit}`).then((response) => ({
        ...response,
        data: {
          data: response.data.map((user: any) => userApi.mapUserData(user)),
          total: response.data.length, // Backend should provide total count
          page,
          limit,
          totalPages: Math.ceil(response.data.length / limit),
